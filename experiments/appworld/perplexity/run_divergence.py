...
Reuses the cached perplexity artifacts:
  * segmentation + cumulative summaries  <- perplexity/compressions/<tag>/<task>.json
  * the real trajectories                <- <trajectory_dir>/task_<id>/llm_history.json

Generated plans and judge scores are cached to
``perplexity/outputs/divergence_cache/<tag>/<task>.json`` so re-runs are cheap.

Example:
    cd experiments/appworld
    export VLLM_BASE_URL=http://r4519u01n01:8000/v1
    python -m perplexity.run_divergence \
...
def parse_args():
    ...
    p.add_argument("--compressions_dir", required=True,
                   help="Cached cumulative compressions dir (perplexity/compressions/<tag>)")
