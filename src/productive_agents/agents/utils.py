...
class LLMManager:
    ...
    @staticmethod
    def create_llm(model_name: str, key: str, system_message: str, lora_name: Optional[str] = None, port: Optional[int] = None, base_url: Optional[str] = None):
        """Create appropriate LLM instance based on model name."""
        # ChatGPT-family detection. "gpt-oss" should route to vLLM, not OpenAI.
        is_openai = (
            any(pattern in model_name for pattern in ['gpt', 'o1', 'o3', 'o4'])
            and 'gpt-oss' not in model_name.lower()
        )
        if is_openai:
            ...
            return vLLM(model_name, system_message, lora_name=lora_name, port=port, base_url=base_url)
